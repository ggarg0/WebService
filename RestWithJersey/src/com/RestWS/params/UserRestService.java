...
public class UserRestService {
	...
	@Produces(MediaType.TEXT_HTML)
	@Path("/defaultquery")
	public Response getUsers1(@DefaultValue("Default Gaurav") @QueryParam("from") String from,
	...
	public Response addUser(@HeaderParam("user-agent") String userAgent,
		...
		if (!isUserAuthenticated(authString)) {
			return Response.status(401).entity("User authentication failed " + authString).build();
		}
	...
	public Response getImageFile() {
		...
		response.header("Content-Disposition", "attachment; filename=image_from_server.jpeg");
	...
	public Response getTextFile() {
		...
		response.header("Content-Disposition", "attachment; filename=Login Data");
		return response.build();
	...
	public Response getPDFFile() {
		...
		ResponseBuilder response = Response.ok((Object) file);
		response.header("Content-Disposition", "attachment; filename=KBB_from_server.pdf");
		return response.build();
